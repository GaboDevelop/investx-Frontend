...
async function getUnidadInformativaById(req, res) {
  try {
    ...
    const sql =
      "SELECT * FROM UNIDAD_INFORMATIVA WHERE idUnidadInformativa = $1";
    const params = [id];
...
async function createUnidadInformativa(req, res) {
  try {
    ...
    const sql =
      "INSERT INTO UNIDAD_INFORMATIVA (idProyectiva, idEvento, idea, cita, referencia) VALUES ($1, $2, $3, $4, $5) RETURNING idUnidadInformativa";
...
async function updateUnidadInformativa(req, res) {
  try {
    ...
    const sql2 =
      "UPDATE UNIDAD_INFORMATIVA SET idProyectiva = $1, idEvento = $2, idea = $3, cita = $4, referencia = $5 WHERE idUnidadInformativa = $5";
...
async function deleteUnidadInformativa(req, res) {
  try {
    ...
    const sql2 =
      "UPDATE UNIDAD_INFORMATIVA SET active = $1 WHERE idUnidadInformativa = $2";
