...
async function getDisciplinas(req, res) {
  try {
    ...
    const Disciplina = await client.query(sql);
    res.json({
      data: Disciplina
    });
  } catch (error) {
...
async function createDisciplina(req, res) {
  try {
    ...
    const sql =
      "INSERT INTO DISCIPLINA (disciplina) VALUES ($1) RETURNING idDisciplina";
...
async function updateDisciplina(req, res) {
  try {
    ...
    const sql2 =
      "UPDATE DISCIPLINA SET disciplina = $1 WHERE idDisciplina = $2";
