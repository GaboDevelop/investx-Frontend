...
async function getFundamentoById(req, res) {
  try {
    ...
    const sql =
      "SELECT * FROM FUNDAMENTO_PROYECTIVO WHERE idFundamentoProyectivo = $1";
    const params = [id];
...
async function createFundamento(req, res) {
  try {
    ...
    const sql =
      "INSERT INTO FUNDAMENTO_PROYECTIVO (idProyectiva, teoria) VALUES ($1, $2) RETURNING idFundamentoProyectivo";
...
async function updateFundamento(req, res) {
  try {
    ...
    const sql2 =
      "UPDATE FUNDAMENTO_PROYECTIVO SET idProyectiva = $1, teoria = $2  WHERE idFundamentoProyectivo = $3";
...
async function deleteFundamento(req, res) {
  try {
    ...
    const sql2 =
      "UPDATE FUNDAMENTO_PROYECTIVO SET active = $1  WHERE idFundamentoProyectivo = $2";
