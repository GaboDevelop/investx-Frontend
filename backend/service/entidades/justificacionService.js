...
async function createJustificacion(req, res) {
    ...
    try {
        const { idTemporalidad, idContexto, idUnidadEstudio, idEvento, idTipoInvestigacion } = req.body;

        const sql = 'INSERT INTO JUSTIFICACION (idTemporalidad, idContexto, idUnidadEstudio, idEvento, idTipoInvestigacion) VALUES ($1, $2, $3, $4, $5)';
        const params = [idTemporalidad, idContexto, idUnidadEstudio, idEvento, idTipoInvestigacion];
        const justificacion = await client.query(sql, params);
        res.json({
            data: justificacion
        })
...
async function updateJustificacion(req, res) {
    ...
    try {
        ...
        if (justificacion.length = 0) { 
            ...
            const sql2 = 'UPDATE JUSTIFICACION SET idTemporalidad = $1, idContexto = $2, idUnidadEstudio = $3, idEvento = $4, idTipoInvestigacion = $5   WHERE idJustificacion = $6';
            const params2 = [idTemporalidad, idContexto, idUnidadEstudio, idEvento, idTipoInvestigacion, id];
